"""Dog-profile-only ranking subgraph."""
...
def build_dog_graph(scoring: ScoringService) -> Graph:
    def rank(state: RecommendationState) -> dict[str, tuple[RankedCandidate, ...]]:
        return rank_for_category(state, scoring, Category.DOG)

    builder: StateGraph[RecommendationState, None, RecommendationState, RecommendationState] = (
        StateGraph(RecommendationState)
    )
    builder.add_node("rank_dog_profiles", rank)
    builder.add_edge(START, "rank_dog_profiles")
    builder.add_edge("rank_dog_profiles", END)
    return builder.compile(name="dog-specialist-v1")
