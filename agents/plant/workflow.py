...
def build_plant_graph(scoring: ScoringService) -> Graph:
    def rank(state: RecommendationState) -> dict[str, tuple[RankedCandidate, ...]]:
        return rank_for_category(state, scoring, Category.PLANT)

    builder: StateGraph[RecommendationState, None, RecommendationState, RecommendationState] = (
        StateGraph(RecommendationState)
    )
    builder.add_node("rank_plant_candidates", rank)
    builder.add_edge(START, "rank_plant_candidates")
    builder.add_edge("rank_plant_candidates", END)
    return builder.compile(name="plant-specialist-v1")
